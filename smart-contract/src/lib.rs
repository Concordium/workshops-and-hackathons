...
struct VotingView {
    ...
    /// it received so far.
    tally: BTreeMap<VotingOption, VoteCount>,
}
...
mod tests {
    ...
    fn make_test_host(options: Vec<String>, end_time: Timestamp) -> TestHost<State> {
    ...
    fn test_vote_with_invalid_index() {
        ...
        claim_eq!(
            res,
            Err(VotingError::InvalidVotingOption),
            "Should throw error because voting option is invalid"
        );
    }
    ...
    fn test_vote_with_valid_index() {
        ...
        let ballots = host
            ...
            .map(|(a, b)| (*a, *b))
            .collect::<Vec<_>>();
        let tally = view(&ctx, &mut host)
            .expect("View entrypoint should work")
            .tally;
        let votes_count_0 = tally.get("A").unwrap();
        let votes_count_1 = tally.get("B");
        claim_eq!(*votes_count_0, 1, "Expect: one vote for option 0");
        claim!(votes_count_1.is_none(), "Expect: no votes for option 1");
        claim_eq!(
            vec![(ACC_0, 0)],
            ballots,
            "Expect: ACCOUNT_0 voted for voting option index 0"
        );

        // Change vote.

        // Set up the parameter.
        let vote_parameter = to_bytes(&"B");
        ctx.set_parameter(&vote_parameter);
        // Call the contract function.
        let result = vote(&ctx, &mut host);
        // Check the result.
        ...
        let tally = view(&ctx, &mut host)
            .expect("View entrypoint should work")
            .tally;
        let votes_count_0 = tally.get("A");
        let votes_count_1 = tally.get("B").unwrap();
        claim!(votes_count_0.is_none(), "Expect: no votes for option 0");
